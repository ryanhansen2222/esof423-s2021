...
public class Homework2 extends DBTest {
    ...
    void selectAllArtistsWithMoreThanOneAlbum(){
        List<Map<String, Object>> results = executeSQL(
                "SELECT * FROM artists WHERE Artistid IN (SELECT Artistid FROM albums GROUP BY albums.Artistid HAVING COUNT(albums.Artistid) > 1)");
    ...
    void selectTheTrackAndAlbumAndArtistForAllTracksLongerThanSixMinutes() {
        List<Map<String, Object>> results = executeSQL(
                "SELECT tracks.Name as TrackName, albums.Title as AlbumTitle, artists.Name as ArtistsName FROM tracks JOIN albums ON tracks.Albumid = albums.Albumid JOIN artists ON albums.Artistid = artists.Artistid WHERE tracks.Milliseconds > 1000 * 60 * 6");
        // NEED JOIN
